...
SqlStore.prototype.populate = function (options, callback) {
  ...
  var tasks = [
    ...
    function (cb) {
      ...
      async.eachSeries(filteredRelationArray, function (model, ecb) {
...
SqlStore.prototype.defineRelationModels = function(allStores, createDb) {
  var self = this;
  ...
  var belongsToRelations = Object.keys(self.resourceConfig.attributes).filter(function (attributeName) {
    var settings = self.resourceConfig.attributes[attributeName]._settings
    if (!settings) return false
    return (settings.__one || settings.__many) && settings.__as
  });
  belongsToRelations = _.pick(self.resourceConfig.attributes, belongsToRelations);
  ...
  Object.keys(relations).forEach(function (relationName) {
    ...
    var otherModel = self._defineRelationModel(relationName, relation._settings.__many, createDb)
    self.relations[relationName] = otherModel
    self.relationArray.push(otherModel)
  });

  Object.keys(belongsToRelations).forEach(function (relationName) {
    var relation = belongsToRelations[relationName];
    var otherModel = self._defineBelongsToModel(relationName, relation._settings);
    self.relations[relationName] = otherModel;
    self.relationArray.push(otherModel);
  });
}

SqlStore.prototype._defineBelongsToModel = function (relationName, settings) {
  let self = this;
  let inverseRelation = settings.__many ?  _.first(settings.__many) : _.first(settings.__one);
  let modelName = self.resourceConfig.resource + '-' + relationName;
  ...
  let modelProperties = {
    ...
    id: {
      ...
      allowNull: true,
      field: pluralize( inverseRelation ) + 'Id'
    },
    type: {
      type: DataTypes.STRING,
      set: function(/* val */) { this.setDataValue('type', pluralize(self.resourceConfig.resource)); },
      get: function(){ return pluralize(inverseRelation) },
    },
    meta: {
      type: DataTypes.STRING,
      get: function () {
  ...
  modelProperties[pluralize( self.resourceConfig.resource ) + 'Id' ] = {
    type: new DataTypes.STRING(38),
    allowNull: true,
    field: 'id'
  };
  ...
  var relatedModel = self.sequelize.define(modelName, modelProperties, {
    ...
    tableName: `${inverseRelation}-${settings.__as}`
  })

  if (settings.__many) {
    self.baseModel.hasMany(relatedModel, {foreignKey: pluralize(self.resourceConfig.resource) + 'Id' });
  } else {
    self.baseModel.hasOne(relatedModel, { foreignKey: pluralize(self.resourceConfig.resource) + 'Id' });
  }
  relatedModel['isBelongsTo'] = true;
  if (inverseRelation == self.resourceConfig.resource){
    relatedModel['selfRelationProp'] =  pluralize(inverseRelation) + 'Id';
  }

  return relatedModel;
}

SqlStore.prototype._defineRelationModel = function (relationName, many, createDb) {
  var self = this
  ...
  let settings = self.resourceConfig.attributes[relationName]._settings;
  ...
  // When initializing a new DB we temporarily create these relationships in order to create bi-directional cascading delete constraints
  // for join tables. When initializing in server mode, we don't want them because they break some belongs-to relationships
  if (createDb) {
    if (many){
      relationName = _.first(settings['__many']);
    } else {
      relationName = _.first(settings['__one']);
    }
    self.allStores[pluralize(relationName)].baseModel.hasMany(relatedModel, { onDelete: 'CASCADE', foreignKey: 'id' });
  }
...
SqlStore.prototype._generateSearchIncludes = function (relationships) {
  ...
  var searchIncludes = Object.keys(self.relations).reduce(function (partialSearchIncludes, relationName) {
    ...
    let whereBlock = {};
    let whereAttribute = model.selfRelationProp ? model.selfRelationProp : 'id';
    whereBlock[whereAttribute] = matchingValue;
    let includeClause = {
      model: model,
      where: whereBlock
    }
...
SqlStore.prototype._generateSearchBlock = function (request) {
  ...
  var filterBooleans = self._getFilterBooleans(request.params.filter);
  var searchBlock = self._getSearchBlock(attributesToFilter, filterBooleans);
  return searchBlock;
}

SqlStore.prototype._getFilterBooleans = function (requestFilter) {
  if (!requestFilter || !requestFilter.filterBooleans) return [];
  let filterBooleans = _.omit(requestFilter.filterBooleans, Object.keys(this.relations))
  Object.keys(filterBooleans).forEach(key => {
      let val = filterBooleans[key];
      if (Array.isArray(val) && val[0] != null) val = val[0];
      if ('AND' === val) val = Op.and;
      if ('OR' === val) val = Op.or;
...
SqlStore.prototype._filterElementToSearchBlock = function (filterElement, booleanOperator = Op.or) {
  var self = this
  ...
  return { [booleanOperator]: whereObjs }
}

SqlStore.prototype._getSearchBlock = function (filter, filterBooleans) {
  var self = this
  ...
  Object.keys(filter).forEach(function (attributeName) {
    let filterElement = filter[attributeName];
    let filterBoolean = filterBooleans[attributeName] ?  filterBooleans[attributeName] : Op.or;
    searchBlock[attributeName] = self._filterElementToSearchBlock(filterElement, filterBoolean)
  })
...
SqlStore.prototype.search = function (request, callback) {
 ...
 //Extend data store handler prototype with findOne() to find single record by filter
 SqlStore.prototype.findOne = async function(attributeFilters) {
  delete attributeFilters.type;
  var result = await this.baseModel.findOne({
    where: attributeFilters
  });
  ...
  self._dealWithTransaction(finishedCallback, function (t, finishTransaction) {
    self.baseModel.findOne({
      ...
      self._clearAndSetRelationTables(theResource, partialResource, t, function (err) {
        ...
        //For some reason the framework sets the "meta" key but leaves it undefined
        //and this triggeres a subtle Sequelize bug if all other updated values are unchanged from the previous state
        //https://stackoverflow.com/questions/48061748/query-was-empty-nodejs-sequlize
        if (undefined === partialResource.meta) {
